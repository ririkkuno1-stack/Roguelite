...
namespace Core.MasterData
{
    ...
    {
        public const string ENEMY_LABL = "EnemyData";
        public const string WEAPON_LABL = "WeaponData";
        public const string SKILL_LABEL = "SkillData";
        ...
        public async UniTask InitializeAsync()
        {
            await UniTask.WhenAll(
                LoadAsync<EnemyData,EnemyDataRecord>(ENEMY_LABL),
                LoadAsync<WeaponData, WeaponDataRecord>(WEAPON_LABL),
        ...
        private async UniTask LoadAsync<TAsset, TRecord>(string label)
            where TAsset : ScriptableObject, MasterDataContainer<TRecord>
            where TRecord : IMsterData
        {
        ...
        /// </summary>
        public TRecord GetById<TRecord>(ulong id) where TRecord : IMsterData
        {
        ...
        public TRecord GetRandom<TRecord>() where TRecord : IMsterData
        {
            if (masterDataDictionaries.ContainsKey(typeof(TRecord)))
            {
                var dict = (Dictionary<ulong, TRecord>)masterDataDictionaries[typeof(TRecord)];
        ...
        /// </summary>
        public IReadOnlyCollection<TRecord> GetAll<TRecord>() where TRecord : IMsterData
        {
            if (masterDataDictionaries.ContainsKey(typeof(TRecord)))
            {
                var dict = (Dictionary<ulong, TRecord>)masterDataDictionaries[typeof(TRecord)];
        ...
        /// </summary>
        public int Count<TRecord>() where TRecord : IMsterData
        {
            if (masterDataDictionaries.ContainsKey(typeof(TRecord)))
            {
                var dict = (Dictionary<ulong, TRecord>)masterDataDictionaries[typeof(TRecord)];
